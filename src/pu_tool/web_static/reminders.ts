...
import { describeReminderTime } from './reminder_time';
...
let loading = false;
let generation = 0;
let countdownTargets: Array<{ node: HTMLElement; status: HTMLElement; start: number | null; end: number | null }> = [];
...
function updateCountdowns() {
  ...
  countdownTargets.forEach((target) => {
    const state = describeReminderTime(target.start, target.end, now);
    target.status.textContent = state.status;
    target.node.textContent = state.countdown;
  });
}
function render(items: Reminder[]) {
  ...
  sorted.forEach((item, index) => {
    ...
    const head = make('div', undefined, { class: 'card-head' });
    const status = make('span', undefined, { class: 'status' });
    head.append(make('h2', title), status);
    card.append(head);
    const when = start === null ? '时间未知' : new Date(start).toLocaleString() + (end === null ? '' : ' - ' + new Date(end).toLocaleTimeString());
    const meta = make('div', undefined, { class: 'reminder-meta' });
    const countdown = make('strong', undefined, { class: 'reminder-countdown' });
    meta.append(countdown, make('span', '时间：' + when), make('span', '地点：' + String(item.location || '未提供')));
    card.append(meta);
    countdownTargets.push({ node: countdown, status, start, end });
...
if (root) {
  ...
  countdownTimer = window.setInterval(updateCountdowns, 15000);
  const visibility = () => { if (document.visibilityState === 'visible') { updateCountdowns(); void load(); } };
